...
SC_MODULE(rv_thunder) {
	...
	sc_signal<sc_uint<1>> branchsig, pc_up,inval_add,inv_add,orout,res_add,csr_pcsel;
	...
	SC_CTOR(rv_thunder) :pc_inst("pc_inst"), fetch_inst("fetch_inst"), decode_inst("DecodeRinstance"), reg_inst("reginst"), mux_inst("mux_inst"), alu_inst("alu_inst"), data_mem_inst("data_mem_inst"), mux_inst1("mux_inst1"), mux2bit_inst("mux2bit_inst"), branch_inst("branch_inst"), mux_inst2("mux_inst2"), csr_inst("csr_inst"), mux_inst3("mux_inst3"),or_inst("or_inst") {
		...
		fetch_inst.inval_add(inval_add);
...
int sc_main(int argc, char* argv[]) {
	...
	sc_trace(tf, TOP.decode_inst.trig, "trig");
	...
	sc_trace(tf, TOP.data_mem_inst.inv_add, "inv_add");
	...
	sc_trace(tf, TOP.csr_inst.csr_sig, " csr_sig");
	sc_trace(tf, TOP.csr_inst.csr_reg, " csr_value");
	sc_trace(tf, TOP.csr_inst.rs1_data, " csr_rs1_data");
	...
	sc_trace(tf, TOP.csr_inst.mis_all_ins, " mis_all_ins");
	sc_trace(tf, TOP.csr_inst.mtvec_rec, " mtvec_reg");
	sc_trace(tf, TOP.csr_inst.mepc_reg, " mepc_reg");
	sc_trace(tf, TOP.csr_inst.pc_out, " pc_out_csr");
	sc_trace(tf, TOP.csr_inst.pc_sel, "csr_pcsel");
	sc_trace(tf, TOP.csr_inst.ill_instr, "illegal_instr");
	sc_trace(tf, TOP.csr_inst.mret, "mret_csr");
